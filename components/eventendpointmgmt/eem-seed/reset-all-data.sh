...
# Based on https://github.com/IBM/event-automation-demo with addition for specifying EEM and ES CR names.
EEM_MANAGER_CR_NAME=eventendpointmgr
GATEWAY_GROUP=gwgroup1

KAFKA_CLUSTER_CR_NAME=minimal-prod
KAFKA_USER_SECRET=scram-user
KAFKA_BOOTSTRAP_PORT=9093
KAFKA_LISTENER_NAME=tls
...
EEM_API=$(oc get route -n $NAMESPACE $EEM_MANAGER_CR_NAME-ibm-eem-admin -ojsonpath='https://{.spec.host}')
...
function reset_eem() {
    echo " ----------------------------------------------------------------------- "
    echo " This will CLEAR ALL STORAGE for the $EEM_MANAGER_CR_NAME instance "
    ...
    oc exec -it -n $NAMESPACE $EEM_MANAGER_CR_NAME-ibm-eem-manager-0 -- rm -rf /opt/storage/eem
    oc delete pod -n $NAMESPACE $EEM_MANAGER_CR_NAME-ibm-eem-manager-0
    # waiting for replacement pod to be created
    until oc get pod -n $NAMESPACE $EEM_MANAGER_CR_NAME-eem-manager-0 >/dev/null 2>&1; do
        sleep 1
    done
    # waiting for replacement pod to become ready
    oc wait -n $NAMESPACE --for=condition=ready pod $EEM_MANAGER_CR_NAME-ibm-eem-manager-0 --timeout=120s
}
...
JSON_PATH='{.status.kafkaListeners[?(@.name=="'$KAFKA_LISTENER_NAME'")].certificates[0]}' 
ES_CERTIFICATE=$(oc get eventstreams $KAFKA_CLUSTER_CR_NAME -n $NAMESPACE -o jsonpath=$JSON_PATH)
ES_CERTIFICATE=${ES_CERTIFICATE//$'\n'/\\\\n}

# get the password for the Kafka cluster
ES_PASSWORD=$(oc get secret $KAFKA_USER_SECRET -n $NAMESPACE -ojsonpath='{.data.password}' | base64 -d)

# substitute details into the template
cat eem-01-cluster.json | \
    sed "s|ES_NAMESPACE|$NAMESPACE|" | \
    sed "s|KAFKA_CLUSTER_CR_NAME|$KAFKA_CLUSTER_CR_NAME|" | \
    sed "s|ES_CERTIFICATE|$ES_CERTIFICATE|" | \
    sed "s|9999|$KAFKA_BOOTSTRAP_PORT|" | \
    sed "s|KAFKA_USER_SECRET|$KAFKA_USER_SECRET|" | \
...
(
    ...
    RESPONSE=$(curl -X POST -s -k \
                ...
                $EEM_API/eem/clusters?disableValidation=true)
...
do
    ...
    cat eem-03-$topic-option.json | \
        sed "s|gateway-group|$GATEWAY_GROUP|" | \
