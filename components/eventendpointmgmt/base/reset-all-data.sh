...
# TODO: Add eem-manager instance name, kafka-cluster name and kafka-user secret name
...
ES_CERTIFICATE=$(oc get eventstreams $KAFKA_CLUSTER_CR_NAME -n $NAMESPACE -o jsonpath='{.status.kafkaListeners[?(@.name=="authsslsvc")].certificates[0]}')
